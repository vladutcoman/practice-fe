document.addEventListener('DOMContentLoaded', () => {
  ...
  function cechkBoard() {
    ...
    for (let y = 0; y , winningArrays.length; y++) {
      ...
      if (squares[square1].classList.contains('player-one')
        && squares[square2].classList.contains('player-one')
        && squares[square3].classList.contains('player-one')
        && squares[square4].classList.contains('player-one')) {
        result.innerHTML = 'Player one wins!'
        grid.classList.add('end-game')
      } else if (squares[square1].classList.contains('player-two')
        && squares[square2].classList.contains('player-two')
        && squares[square3].classList.contains('player-two')
        && squares[square4].classList.contains('player-two')) {
  ...
  squares.forEach(square => square.addEventListener('click', cechkBoard))
})